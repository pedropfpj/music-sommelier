...
async function listPosts(req, res, config, access) {
  try {
    ...
    const posts = feed.posts
      ...
      .filter((post) => !blocked.has(String(post.userId || "")))
...
async function createPost(req, res, config, access, body) {
  ...
  const context = trimText(body.context || body.meta || "", MAX_META_LENGTH);
  if (!postBody) {
    sendJson(req, res, 400, { ok: false, error: "missing_body" }, ["GET", "POST", "DELETE", "OPTIONS"]);
    return;
  }
  const moderation = assessUserGeneratedText([title, postBody, context]);
...
async function deletePost(req, res, config, access, body) {
  ...
  try {
    ...
    const post = feed.posts.find((item) => item.id === postId && !item.deletedAt && (canModerate || item.userId === access.userId));
    if (!post) {
      sendJson(req, res, 404, {
        ok: false,
        viewer: viewerContext(access),
    ...
    post.deletedBy = access.userId;
    post.moderated = Boolean(canModerate && post.userId !== access.userId);
...
async function updatePost(req, res, config, access, body) {
  ...
  try {
    ...
    const post = feed.posts.find((item) => item.id === postId && !item.deletedAt);
    const canModerate = Boolean(access.canModerate || access.owner || access.moderator);
    if (!post || (!canModerate && post.userId !== access.userId)) {
      sendJson(req, res, 404, {
        ok: false,
        viewer: viewerContext(access),
        error: "post_not_found"
    ...
    const nextTitle = body.title !== undefined
      ? trimText(body.title || "", MAX_TITLE_LENGTH) || post.title || "Conversa da comunidade"
      : post.title;
    const nextBody = body.body !== undefined
      ? trimText(body.body || body.text || "", MAX_BODY_LENGTH)
      : post.body;
    const nextContext = body.context !== undefined || body.meta !== undefined
      ? trimText(body.context || body.meta || "", MAX_META_LENGTH)
      : post.context;
    const moderation = assessUserGeneratedText([nextTitle, nextBody, nextContext]);
    if (!moderation.allowed) {
      sendJson(req, res, 422, {
        ok: false,
        error: "content_not_allowed",
...
async function reportPost(req, res, config, access, body) {
  if (!access.authenticated || !access.userId) {
    sendJson(req, res, 401, { ok: false, error: "login_required" }, ["GET", "POST", "DELETE", "OPTIONS"]);
    return;
  }
  ...
  try {
    ...
    const metadata = body.metadata && typeof body.metadata === "object" ? body.metadata : {};
    const reporter = authorFromMetadata(access, metadata);
...
async function updateBlockedUser(req, res, config, access, body) {
  ...
  if (!targetUserId || targetUserId === access.userId) {
    sendJson(req, res, 400, { ok: false, error: "invalid_block_target" }, ["GET", "POST", "DELETE", "OPTIONS"]);
    return;
  }
  try {
...
module.exports = async function handler(req, res) {
  ...
  if (!requireMusicApi(req, res, {
    ...
    feature: "community",
    enabledEnv: "SONIC_COMMUNITY_ENABLED",
    defaultEnabled: true,
    allowGlobalFallback: false,
    dailyLimitEnv: "SONIC_COMMUNITY_DAILY_LIMIT",
  ...
  if (req.method === "POST") {
    ...
    if (action === "report") {
      await reportPost(req, res, config, access, body);
      return;
    }
    if (action === "block") {
