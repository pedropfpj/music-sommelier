...
module.exports = async function handler(req, res) {
  ...
  const row = parsedV2?.row || {
    event_id: cleanUuid(body.event_id ?? body.eventId) || null,
    anonymous_id: cleanUuid(body.anonymous_id ?? body.anonymousId) || null,
    occurred_at: cleanTimestamp(body.occurred_at ?? body.occurredAt) || null,
    schema_version: schemaVersion,
    platform: /^(web|ios|android|native)$/.test(trimText(body.platform, 20).toLowerCase())
      ? trimText(body.platform, 20).toLowerCase()
      : null,
    app_version: trimText(body.app_version ?? body.appVersion, 80) || null,
    environment: /^(development|preview|production|test)$/.test(trimText(body.environment, 20).toLowerCase())
      ? trimText(body.environment, 20).toLowerCase()
      : null,
