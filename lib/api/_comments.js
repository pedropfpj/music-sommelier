...
async function deleteCommentInStorage(req, res, config, access, targetType, targetKey, commentId) {
  try {
    ...
    thread.comments = thread.comments.filter((row) => !(row.id === commentId && row.userId === access.userId));
    if (thread.comments.length === before) throw new Error("comment_not_found");
    await writeStorageThread(config, thread);
    sendJson(req, res, 200, { ok: true, storageFallback: true }, ["GET", "POST", "PATCH", "DELETE", "OPTIONS"]);
    return true;
  } catch (error) {
    sendJson(req, res, 200, {
      ok: false,
...
async function listComments(req, res, config, access) {
  ...
  try {
    ...
    const profiles = await fetchProfiles(config, comments.map((row) => row.user_id));
    const reactions = await fetchReactionSummary(config, comments.map((row) => row.id), access.userId);
    sendJson(req, res, 200, {
      ok: true,
      enabled: true,
      targetType,
...
async function updateComment(req, res, config, access, body) {
  ...
  try {
    ...
    sendJson(req, res, 500, { ok: false, error: "comment_update_failed", detail: error.message }, ["GET", "POST", "PATCH", "DELETE", "OPTIONS"]);
  }
}
...
async function deleteComment(req, res, config, access, body) {
  ...
  try {
    ...
    await supabaseRest(config, `comments?${params.toString()}`, {
      method: "DELETE",
      prefer: "return=minimal"
    });
    sendJson(req, res, 200, { ok: true }, ["GET", "POST", "PATCH", "DELETE", "OPTIONS"]);
  } catch (error) {
    if (setupNeededFromError(error.message)) {
      const { targetType, targetKey } = requestTargetFromBodyOrQuery(req, body);
    ...
    sendJson(req, res, 500, { ok: false, error: "comment_delete_failed", detail: error.message }, ["GET", "POST", "PATCH", "DELETE", "OPTIONS"]);
  }
}
