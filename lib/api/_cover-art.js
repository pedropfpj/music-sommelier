...
async function resolveDeezerCoverArt(request = {}) {
  ...
  const query = `artist:"${artist.replace(/"/g, "")}" ${request.album ? "album" : "track"}:"${title.replace(/"/g, "")}"`;
  ...
  const candidates = rows
    .map((row) => {
      ...
      const candidateTitle = request.album ? row?.album?.title : row?.title_short || row?.title;
      ...
      return { row, score, candidateTitle };
    })
    .filter((candidate) => candidate.score >= 0.62)
    .sort((a, b) => b.score - a.score);
  for (const candidate of candidates) {
    const album = candidate.row?.album || {};
