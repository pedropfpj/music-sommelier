...
const DEEZER_SOURCE_DETAIL = {
  name: "Deezer",
  provider: "Deezer Search API",
  url: "https://www.deezer.com/",
  dataType: "release cover artwork metadata",
  license: "Cover images remain copyrighted by their respective owners",
  attribution: "Fallback cover metadata and hosted artwork from Deezer.",
  backendOnly: true,
  cached: true
};
...
async function resolveDeezerCoverArt(request = {}) {
  const artist = trimText(request.artist, 140);
  const title = trimText(request.album || request.song, 180);
  if (!artist || !title) return null;
  const expectedArtist = normalizeText(artist);
  const plainQuery = `${artist.replace(/"/g, "")} ${title.replace(/"/g, "")}`;
  const endpoint = request.album ? DEEZER_ALBUM_SEARCH_URL : DEEZER_SEARCH_URL;
  const params = new URLSearchParams({ q: plainQuery, limit: "12" });
  const payload = await fetchJson(`${endpoint}?${params.toString()}`);
  const primaryRows = Array.isArray(payload?.data) ? payload.data : [];
  let candidates = primaryRows
    .map((row) => {
      const album = request.album ? row : row?.album || {};
      const actualArtist = normalizeText(row?.artist?.name || "");
      const candidateTitle = request.album ? row?.title : row?.title_short || row?.title;
      ...
      const score = artistMatches ? titleScore(title, candidateTitle) : 0;
      return { row, album, score, candidateTitle };
    })
    .filter((candidate) => candidate.score >= 0.62)
    .sort((a, b) => b.score - a.score);

  // Album search is the most precise source, but a generic track search is a
  // useful fallback when Deezer has indexed the release only through tracks.
  if (!candidates.length && request.album) {
    const trackPayload = await fetchJson(`${DEEZER_SEARCH_URL}?${params.toString()}`);
    const trackRows = Array.isArray(trackPayload?.data) ? trackPayload.data : [];
    candidates = trackRows
      .map((row) => {
        const album = row?.album || {};
        const actualArtist = normalizeText(row?.artist?.name || "");
        const candidateTitle = album.title || "";
        const artistMatches = actualArtist === expectedArtist || actualArtist.includes(expectedArtist) || expectedArtist.includes(actualArtist);
        const score = artistMatches ? titleScore(title, candidateTitle) : 0;
        return { row, album, score, candidateTitle };
      })
  ...
  for (const candidate of candidates) {
    ...
    const imageUrl = trimText(album.cover_xl || album.cover_big || album.cover_medium || album.cover || "", 1200);
    if (!imageUrl) continue;
    return {
      id: String(album.id || candidate.row?.id || ""),
      type: "deezer-album",
      title: trimText(album.title || candidate.candidateTitle, 180),
      imageUrl,
      sourceUrl: album.id ? `https://www.deezer.com/album/${encodeURIComponent(album.id)}` : trimText(candidate.row?.link, 1200),
      confidenceScore: Math.min(1, Math.max(0, candidate.score)),
      source: "deezer"
    };
  }
  return null;
...
async function resolveCoverArt(request = {}) {
  ...
  return resolveDeezerCoverArt(request);
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const source = cover?.source === "deezer" ? "deezer" : "cover-art-archive";
    const sourceDetails = source === "deezer" ? [DEEZER_SOURCE_DETAIL, SOURCE_DETAIL] : [SOURCE_DETAIL, DEEZER_SOURCE_DETAIL];
    const payload = {
      ok: true,
      enabled: true,
      source,
    ...
    // Do not keep a transient upstream miss for a full day. Successful covers
    // remain cached; misses can recover on the next request/provider attempt.
    if (cover?.imageUrl) cacheWrite(key, payload);
    if (cover?.imageUrl) {
