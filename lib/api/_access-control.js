...
function configuredModeratorEmails(ownerEmails = configuredOwnerEmails()) {
  return new Set([...ownerEmails, ...csvEmails(envText("SONIC_MODERATOR_EMAILS", ""))]);
}
...
async function resolveAccessContext(req, body = {}) {
  ...
  const moderatorEmails = configuredModeratorEmails(ownerEmails);
  const premiumEmails = configuredPremiumEmails();
  const owner = Boolean(email && ownerEmails.has(email));
  const moderator = Boolean(owner || (email && moderatorEmails.has(email)));
  ...
  return {
    ...
    premium,
    role: owner ? "owner" : moderator ? "moderator" : premium ? "premium" : email ? "free" : "anonymous"
  };
}
