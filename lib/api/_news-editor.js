...
const TRANSLATIONS_STORAGE_KEY = "_sonicTranslations";
const PUBLIC_FIELDS = [
  "id",
  "slug",
  "status",
...
function compactLocalizedObject(value) {
  if (Array.isArray(value)) {
    return value.map(compactLocalizedObject).filter((entry) => {
...
function normalizeLocalizedMedia(value) {
  const input = Array.isArray(value) ? value : [];
  const output = input.slice(0, 8).map((entry) => compactLocalizedObject({
    alt: trimText(entry?.alt, 240),
    caption: trimText(entry?.caption, 320),
    credit: trimText(entry?.credit, 180)
  }));
  while (output.length && !Object.keys(output[output.length - 1]).length) output.pop();
  return output;
}

function normalizeLocalizedLinks(value) {
  const input = Array.isArray(value) ? value : [];
  const output = input.slice(0, 8).map((entry) => compactLocalizedObject({
    label: trimText(entry?.label, 80)
  }));
...
function normalizeLocalizedLineup(value) {
  const input = Array.isArray(value) ? value : [];
  const output = input.slice(0, 80).map((entry) => compactLocalizedObject({
    name: trimText(entry?.name, 120),
    time: trimText(entry?.time, 40),
    role: trimText(entry?.role, 80)
  }));
...
function normalizeLocalizedServiceInfo(value) {
  const info = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return compactLocalizedObject({
    address: trimText(info.address, 280),
    price: trimText(info.price, 160),
    ageRating: trimText(info.ageRating, 80),
    doorsTime: trimText(info.doorsTime, 160),
    instagramLabel: trimText(info.instagramLabel, 80),
    notes: longText(info.notes, 1200),
    media: normalizeLocalizedMedia(info.media),
    relatedLinks: normalizeLocalizedLinks(info.relatedLinks)
  });
}

function normalizeArticleTranslation(value) {
  const translation = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return compactLocalizedObject({
    category: trimText(translation.category, 80),
    eyebrow: trimText(translation.eyebrow, 80),
    title: trimText(translation.title, 180),
    excerpt: longText(translation.excerpt, 520),
    body: longText(translation.body, 24000),
    coverImageAlt: trimText(translation.coverImageAlt, 240),
    imageCredit: trimText(translation.imageCredit, 180),
    authorName: trimText(translation.authorName, 120),
    venueName: trimText(translation.venueName, 180),
    city: trimText(translation.city, 100),
    state: trimText(translation.state, 80),
    lineup: normalizeLocalizedLineup(translation.lineup),
    serviceInfo: normalizeLocalizedServiceInfo(translation.serviceInfo),
    ctaLabel: trimText(translation.ctaLabel, 60),
    disclosure: longText(translation.disclosure, 320)
  });
}

function normalizeArticleTranslations(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return Object.fromEntries(["en", "es"].map((language) => [
    language,
    normalizeArticleTranslation(source[language])
  ]).filter(([, translation]) => Object.keys(translation).length));
}
...
function articleInput(body = {}, { partial = false, userId = "" } = {}) {
  ...
  const hasServiceInfo = !partial || Object.prototype.hasOwnProperty.call(source, "serviceInfo");
  const hasTranslations = !partial || Object.prototype.hasOwnProperty.call(source, "translations");
  if (hasServiceInfo || hasTranslations) {
    const serviceInfo = normalizeServiceInfo(source.serviceInfo);
    const translations = normalizeArticleTranslations(source.translations);
    if (Object.keys(translations).length) serviceInfo[TRANSLATIONS_STORAGE_KEY] = translations;
    output.service_info = serviceInfo;
  }
...
function publicArticle(row = {}) {
  const storedServiceInfo = row.service_info && typeof row.service_info === "object" && !Array.isArray(row.service_info)
    ? row.service_info
    : {};
  const { [TRANSLATIONS_STORAGE_KEY]: embeddedTranslations, ...serviceInfo } = storedServiceInfo;
  const translations = row.translations && typeof row.translations === "object" && !Array.isArray(row.translations)
    ? row.translations
    : embeddedTranslations && typeof embeddedTranslations === "object" && !Array.isArray(embeddedTranslations)
...
async function listArticles(req, res, config) {
  ...
  const limit = Math.max(1, Math.min(100, Number.parseInt(queryValue(req, "limit") || "20", 10) || 20));
  const offset = Math.max(0, Math.min(100000, Number.parseInt(queryValue(req, "offset") || "0", 10) || 0));
  ...
  sendJson(req, res, 200, {
    ok: true,
    articles: visibleRows.map(publicArticle),
    pagination: {
      limit,
      offset,
      hasMore,
...
async function updateArticle(req, res, config, body) {
  ...
  delete article.slug;
  const source = body?.article && typeof body.article === "object" ? body.article : body;
  const updatesServiceInfo = Object.prototype.hasOwnProperty.call(source || {}, "serviceInfo");
  const updatesTranslations = Object.prototype.hasOwnProperty.call(source || {}, "translations");
  if (updatesServiceInfo !== updatesTranslations) {
    const currentParams = new URLSearchParams({ id: `eq.${id}`, select: "service_info", limit: "1" });
    const currentRows = await supabaseRequest(config, newsTablePath(currentParams), { method: "GET" });
    const currentServiceInfo = currentRows?.[0]?.service_info && typeof currentRows[0].service_info === "object"
      ? currentRows[0].service_info
      : {};
    if (updatesTranslations && !updatesServiceInfo) {
      const incomingTranslations = article.service_info?.[TRANSLATIONS_STORAGE_KEY] || {};
      article.service_info = {
        ...normalizeServiceInfo(currentServiceInfo),
        ...(Object.keys(incomingTranslations).length ? { [TRANSLATIONS_STORAGE_KEY]: incomingTranslations } : {})
      };
    } else if (updatesServiceInfo && !updatesTranslations && currentServiceInfo[TRANSLATIONS_STORAGE_KEY]) {
      article.service_info[TRANSLATIONS_STORAGE_KEY] = currentServiceInfo[TRANSLATIONS_STORAGE_KEY];
    }
  }
